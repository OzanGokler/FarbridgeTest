...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float RunSpeedMultiplier = 5;        // Run speed multiplier
    public Feedback feedback;                   // Click Feedback Object
    ...
    private float agentWalkSpeed;               // The agent walkspeed for single click walk event
    private float agentRunSpeed;                // The agent walkspeed for double click walk event
    ...
    private void Start()
    {
        ...
        agentWalkSpeed = agent.speed; // Set the agent walk speed
        agentRunSpeed = agent.speed * RunSpeedMultiplier; // Set the agent run speed
    }
    ...
    public void OnGroundClick(BaseEventData data)
    {
        ...
        //Determine if double click, and set the agent speed.
        agent.speed = pData.clickCount >= 2 ? agentRunSpeed : agentWalkSpeed;
        
        
        // Set the destination of the nav mesh agent to the found destination position and start the nav mesh agent going.
        agent.SetDestination(destinationPosition);
        feedback.ShowFeedBack(destinationPosition);
        agent.isStopped = false;
    }


    // This function is called by the EventTrigger on an Interactable, the Interactable component is passed into it.
    public void OnInteractableClick(BaseEventData data)
    {
        
        PointerEventData pData = (PointerEventData)data;

        Interactable interactable = pData.pointerPress.GetComponent<Interactable>();
